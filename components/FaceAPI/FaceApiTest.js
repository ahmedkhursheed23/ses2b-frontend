...
function Face(props) {
  ...
  let Photo;
  let user;
  //boolean for if any face is detected
  const [isFace, setFace] = React.useState(false);
  ...
  return (
    <div>
      ...
      <div>
        {isFace ?
          <div>
            <h1>Face Authentication Successful</h1>
            {/* Extra verification step to make sure its the right person */}
            <p>Are you (Students full name will show here)</p>
            <button onClick= {() => router.push("/dashboard/testScreenShare")}>
              Confirm
            </button>
          </div>
          :
